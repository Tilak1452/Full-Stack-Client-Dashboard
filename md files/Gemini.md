...
│       │   ├── __init__.py
│       │   ├── stock_service.py          ← yFinance wrapper: price, history, indicators (15KB)
...
│       │   ├── portfolio_service.py      ← Portfolio CRUD + transaction recording
...
│           ├── portfolio.api.ts          ← portfolioApi: list(), create(), getSummary(), etc.
...
6. On startup: validates DB connection, runs `Base.metadata.create_all()`, starts APScheduler
7. On shutdown: stops APScheduler
...
| `POST` | `/portfolios/{id}/holdings` | Add or update a holding |
| `POST` | `/portfolios/{id}/transactions` | Record a BUY or SELL transaction |
| `GET` | `/portfolios/{id}/summary` | Get aggregated portfolio summary |
...
| `portfolio.api.ts` | `portfolioApi` | `list()`, `create(payload)`, `getSummary(id)`, `addHolding(id, payload)`, `recordTransaction(id, payload)`, `optimize(id)` |
...
- `add_holding(db, portfolio_id, symbol, quantity, price)` → Upserts holding
- `record_transaction(db, portfolio_id, symbol, transaction_type, quantity, price)` → Creates immutable transaction + updates holding
- `get_portfolio_summary(db, portfolio_id)` → Aggregated summary with `total_invested`
...
8. **Watchlist is client-side only:** The watchlist is stored in `localStorage` under key `finsight_watchlist` as a JSON array of symbols. There is no backend persistence for the watchlist.
