...
def create_alert(body: CreateAlertRequest, current_user: dict = Depends(get_current_user)):
    ...
    alert = alert_service.create_alert(body.symbol, body.condition, body.threshold, current_user["id"])
    return {"id": alert.id, "symbol": alert.symbol, "condition": alert.condition, "threshold": alert.threshold, "status": alert.status}


@router.get("/active")
def list_active_alerts(current_user: dict = Depends(get_current_user)):
    """List all currently active alert rules."""
    alerts = alert_service.get_all_active_alerts(current_user["id"])
    return [
        {
...
def get_notifications(current_user: dict = Depends(get_current_user)):
    ...
    return {"notifications": alert_service.get_recent_alerts(current_user["id"])}
